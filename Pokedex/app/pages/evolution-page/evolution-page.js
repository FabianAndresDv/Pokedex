...
class EvolutionPage extends CellsPage {
  ...
  async getBulbasaurEvolutions() {
    try {
      ...
      console.error('Error al obtener las evoluciones del Bulbasaur:', error);
    }
  }

  async getPokemonDetails(aname) {
    try {
      const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${aname}/`);
      ...
      console.error(`Error al obtener los detalles de ${aname}:`, error);
